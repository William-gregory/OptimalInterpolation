...
class DataLoader():
    """
    simple class to load gridded and raw observation data
    """
    ...
    def __init__(self,
        ...
        Parameters
        ----------
        sat_list: list of str or None, default None. Value is assigned to 'sat_list' attribute
            If None then will set to ["CS2_SAR", "CS2_SARIN", "S3A", "S3B"]
        seasons: list of str or None, default None. Value is assigned to 'season' attribute
            If None then will set to ["2018-2019"]
        grid_res: str or None, default None. Values is assigned to 'grid_res' attribute
            if None then will set to '25km'
        verbose: bool or int, default 1. Specifies the verbose level, which higher values printing more detail
        """
    ...
    def load_obs_data(self,
        ...
        """
        Load Observation Data: loads gridded observation data
        - loads data found in sat_data_dir from files
        - read files with names f"{s}_dailyFB_{grid_res}_{season}_season.pkl", where
        - {s} is a satellite specified in sat_list
        - {grid_res} is a str e.g. '25km' or '50km'
        - {season} os a str e.g. '2018-2019'

        Pickle files expected to containing dictionaries with keys YYYYMMDD and values 2-d array of observations
        Data from pkl files is stored in a DataDict object.

        Parameters
        ----------
        sat_data_dir: str or None, default None.
            Path to directory containing (gridded) observation data
        sat_list: list of str or None. If None use sat_list attribute
        season: str or None, default None.
            If None use 'season' attribute from object. Used only if load_sie is True
        grid_res: str or None, default None.
            If None use 'grid_res' attribute from object. Examples: '25km' or '50km'
        take_dim_intersect: bool, default True.
            If True take only the intersecting dimensions between satellites, i.e. take only common dates

        Returns
        -------
        None

        Assigns dict to 'sats' and 'obs' attributes with
        - sats will be a dictionary with keys from 'sat_list' and values DataDict with dim (y,x,date)
        - (y,x) values are taken from self.aux['y'], self.aux['x'] if present, else will use 1:n where n is the grid size
        - obs is a DataDict made from concatenating sats, dimensions (y,x,date,sat)
    ...
    def load_raw_data(self,
        ...
        - loads data found in raw_data_dir from files
        - files expected to be in the form: raw_along_track_lon_lat_fb_{sat_name}.csv
        - where sat_name is in sat_list e.g. in ['CS2_SAR', 'S3A', ...]
        - files expected to contain columns:
        - ['lon', 'lat', 'fb', 'datetime'] where
        -- 'lon' is longitude
        -- 'lat' is latitude
        -- 'fb' is (raw) freeboard measurement
        -- 'datetime' is the date-time given in YYYY-MM-DD H:M:S format

        csv files can be generated by processing ".proc" files, see examples/read_raw_obs_data_and_save_to_csv.py

        Parameters
        ----------
        raw_data_dir: str or None, default None.
            Path to directory containing (raw) observation data
            If None will use <path_to_package>/data/RAW
        sat_list: list of str or None. If None use sat_list attribute
        season: str or None, default None.
            If None use 'season' attribute from object. Used only if load_sie is True
        ...
        Assigns (flat) DataDict to 'raw_obs' attribute with
        vals being the raw observations and
        dims: ['x', 'y', 'datetime', 'sat']
    ...
    def load_aux_data(self,
        ...
        """
        Load Auxiliary Data: Auxiliary data specifies grid attributes
        - loads data found in aux_data_dir from files
        - read files with names {pre_prefix}{p}_{grid_res}.npy, where
        - {pre_prefix} is "" or "new_" depending on 'get_new' argument
        - {p} in ['lon', 'lat', 'x', 'y']
        - example of {grid_res} values are '25km' or '50km'

        Parameters
        ----------
        aux_data_dir: str or None, default None.
            Path to directory containing auxiliary data
        grid_res: str or None, default None.
            If None use 'grid_res' attribute from object
        season: str or None, default None.
            If None use 'season' attribute from object. Used only if load_sie is True
        load_sie: bool, default True.
            Load SIE (Sea Ice Extent) data?
        load_fyi: bool, default True.
            Load FYI (First Year Ice) data?
        get_new: bool, default False.
            If True add 'new_' prefix to files being read in
        get_bin_center: bool, default True. If True move grid values [(lon,lat) and (x,y)] to their centers,
            otherwise will leave as is (i.e. corresponding to the bin edges)
        legacy_projection: bool, default False. If True use the previous projection (i.e. not EASE2.0), used
            only if get_bin_center is True

        Returns
        -------
        None

        Assigns dict to 'aux' attribute with
        - keys ['lon', 'lat', 'x', 'y']
        - values 'DataDict': each with vals being a 2-d array and
        - dims corresponding grid locations ['grid_loc_0', 'grid_loc_1']
